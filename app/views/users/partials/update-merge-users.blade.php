...
@include('layout.header', ['header' => 'Edit Merge User', 'subtitle' => 'User has been merged, please edit profile below for the new user'])
    ...
    <div class="container__block -narrow">
      {{ Form::model($user, [ 'method' => 'PATCH', 'route' => ['users.update', $user['_id']]]) }}
        ...
        <div class="form-item -padded">
          ...
          {{ Form::text('first_name', $user['first_name'], ['class' => 'text-field']) }}
        </div>
        <div class="form-item -padded">
          {{ Form::label('last_name', 'Last Name', ['class' => 'field-label']) }}
          {{ Form::text('last_name', $user['last_name'], ['class' => 'text-field']) }}
        </div>
        <div class="form-item -padded">
          {{ Form::label('email', 'Email', ['class' => 'field-label']) }}
          {{ Form::text('email', $user['email'], ['class' => 'text-field']) }}
        </div>
        <div class="form-item -padded">
          {{ Form::label('mobile', 'Mobile', ['class' => 'field-label']) }}
          {{ Form::text('mobile', $user['mobile'], ['class' => 'text-field']) }}
        </div>
        <div class="form-item -padded">
          {{ Form::label('birthdate', 'Birthdate', ['class' => 'field-label']) }}
          {{ Form::text('birthdate', $user['birthdate'], ['class' => 'text-field']) }}
        </div>
        <div class="form-item -padded">
          {{ Form::label('addr_street1', 'Address St 1', ['class' => 'field-label']) }}
          {{ Form::text('addr_street1', $user['addr_street1'], ['class' => 'text-field']) }}
        </div>
        <div class="form-item -padded">
          {{ Form::label('addr_street2', 'Address St 2', ['class' => 'field-label']) }}
          {{ Form::text('addr_street2', $user['addr_street2'], ['class' => 'text-field']) }}
        </div>
        <div class="form-item -padded">
          {{ Form::label('addr_city', 'City', ['class' => 'field-label']) }}
          {{ Form::text('addr_city', $user['addr_city'], ['class' => 'text-field']) }}
        </div>
        <div class="form-item -padded">
         {{ Form::label('addr_state', 'State', ['class' => 'field-label']) }}
         {{ Form::text('addr_state', $user['addr_state'], ['class' => 'text-field']) }}
        </div>
        <div class="form-item -padded">
         {{ Form::label('addr_zip', 'Zipcode', ['class' => 'field-label']) }}
         {{ Form::text('addr_zip', $user['addr_zip'], ['class' => 'text-field']) }}
        </div>
        <div class="form-item -padded">
          {{ Form::label('country', 'Country', ['class' => 'field-label']) }}
          {{ Form::text('country', $user['country'], ['class' => 'text-field']) }}
        </div>
